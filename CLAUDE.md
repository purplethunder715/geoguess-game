...
**Hybrid street-view providers (decided 2026-05-08).** `attemptOneLocation` tries Google Street View first when the user has set a Google Maps API key (entered via the in-app settings panel, stored in `localStorage["geoguess.googleMapsApiKey"]`). Google's `StreetViewService.getPanorama` returns the closest pano within `GOOGLE_SEARCH_RADIUS_M` (4 km) using `source: OUTDOOR` (excludes user-uploaded photospheres — equivalent to Mapillary's `is_pano=true`). On a `ZERO_RESULTS` (coverage miss) we transparently fall through to Mapillary. Real Google API errors (`OVER_QUERY_LIMIT`, `REQUEST_DENIED`, etc.) propagate as `Google API: <STATUS>` and `isHttpFailure` counts them toward the same backoff/threshold as Mapillary HTTP errors. The Google key is **not** committed alongside `MAPILLARY_TOKEN` — Brady's Google usage bills against his own Cloud account, so per-user localStorage keeps each player on their own billing. Without a Google key, the code path is identical to before (Mapillary-only). The viewer is a single `<div id="mapillary-viewer">` (id kept for back-compat) that hosts either a `mapillary.Viewer` or a `google.maps.StreetViewPanorama`; `state.viewerSource` tracks which is current and `showImageInViewer` tears down the old one when the source flips between rounds.
