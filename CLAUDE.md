...
**Hybrid street-view providers (decided 2026-05-08).** `attemptOneLocation` tries Google Street View first when the user has set a Google Maps API key (entered via the in-app settings panel, stored in `localStorage["geoguess.googleMapsApiKey"]`, OR auto-loaded from the gitignored `public/config.local.js`). Google's `StreetViewService.getPanorama` returns the closest pano within `GOOGLE_SEARCH_RADIUS_M` (4 km) using `source: OUTDOOR` (excludes user-uploaded photospheres — equivalent to Mapillary's `is_pano=true`). **Sanity cap**: results farther than `GOOGLE_RESULT_MAX_KM` (50 km) from the requested point are treated as `ZERO_RESULTS` — Google's radius param is loose with `OUTDOOR` and otherwise hands back panoramas hundreds of km away (asked for Austria, got Berlin). After a hit, `reverseGeocode` runs against the actual photo coords (Geocoding API; degrades silently to the picked-region name if the API isn't enabled) so the result screen reads "Berlin, Germany" instead of "Somewhere in Austria". On a `ZERO_RESULTS` we transparently fall through to Mapillary. Real Google API errors (`OVER_QUERY_LIMIT`, `REQUEST_DENIED`, etc.) propagate as `Google API: <STATUS>` and `isHttpFailure` counts them toward the same backoff/threshold as Mapillary HTTP errors. The Google key is **not** committed alongside `MAPILLARY_TOKEN` — Brady's Google usage bills against his own Cloud account. Without a Google key, the code path is identical to before (Mapillary-only). The viewer is a single `<div id="mapillary-viewer">` (id kept for back-compat) that hosts either a `mapillary.Viewer` or a `google.maps.StreetViewPanorama`; `state.viewerSource` tracks which is current and `showImageInViewer` tears down the old one when the source flips between rounds.

**Round count is user-configurable.** The start screen has a number input (`#rounds-input`, clamped to `MIN_ROUNDS_PER_GAME` 1 / `MAX_ROUNDS_PER_GAME` 20). `state.roundsPerGame` is set from it on Start click; the prefetch pool, round-transition logic, HUD total (`#round-total`), and final-score display all read from it. `ratingFor(score, max)` accepts an explicit max so the bucket thresholds (90/70/50/30%) scale with the configured round count.

**Game-mode selector.** A `#mode-select` on the start screen toggles between two location pools, written to `state.gameMode` on Start click:

- `random` (default): the existing 15% curated / 85% region-random mix. `CURATED_PROBABILITY` was 0.4 originally — Brady reported every round felt like a famous capital because the curated list leans heavy on capitals + the rural-label issue described above. Lowered to 0.15 in the same fix.
- `capitals`: pulls only from `CAPITAL_LOCATIONS`, the precomputed subset of `CURATED_LOCATIONS` where `isCapital === true` (~35 entries — every UN-recognized capital we curate). Region-random is skipped entirely. Used for guessing only major capitals.

**Country-match bonus.** When `reverseGeocode` set `state.actualCountryCode` (i.e. Google key + Geocoding API both available), `submitGuess` reverse-geocodes the user's pin too and awards a +50 country bonus (`applyCountryBonus` in lib.js, capped so `distancePoints + bonus ≤ 5000`). Without a Google key — or if the Geocoder fails — the bonus silently doesn't apply. Country comparison uses ISO 3166-1 alpha-2 codes (`short_name` from the Geocoder), not long names, so "Czech Republic" vs "Czechia" drift doesn't break the match. Tests in `tests/test.js` cover the cap arithmetic.
