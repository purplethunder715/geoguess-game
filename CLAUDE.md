...
The non-obvious bit: **all 5 rounds are prefetched in parallel at game start** (`primeRoundQueue`), and round 1 takes whichever prefetch resolves first via a **race-based pool** (`createRoundPool`) — not whichever is in slot 0. Each `prepareRound` fires `PARALLEL_ATTEMPTS` (3) location lookups in parallel via `Promise.any`; first one to succeed wins, the others are abandoned. Each individual lookup is a single Mapillary Graph API call with `is_pano=true` inside a fixed `SEARCH_DELTA_DEG` (0.04° ≈ 4 km) bbox, with a `FETCH_TIMEOUT_MS` (5 s) AbortController so a slow API can't hang the prefetch. **We do not fall back to flat (non-pano) images** — the player must be able to look around in 360°, and a flat image would just be a frozen photo. The thumbnail URL is preloaded via `preloadImage()` once a round resolves so the browser already has it cached when the viewer renders. If the Mapillary viewer's `moveTo()` rejects (bad image ID, internal corruption), `showImageInViewer` tears the viewer down and rebuilds — without that, the player gets stuck on the previous round's panorama.
